...
function VitalConfigs() {
  return (
    <div className="h-full flex flex-col gap-y-3">
      ...
      <div className="flex flex-col gap-y-12 bg-white rounded-2xl p-6 h-4/6 shadow-lg ">
        <RangeSlider min={60} max={90} label="diastolic" />
        <RangeSlider min={60} max={90} label="systolic" />
        <RangeSlider min={60} max={90} label="pulse" />
        <button className="border-none bg-purple-300 p-3 rounded-xl float-right ml-auto">
          Confirm Changes
        </button>
      </div>
    </div>
